...
- 发布侧频控（`backend/app/ratelimit.py`）：动态 10 条 / 10 分钟、回声 20 条 / 10 分钟、AI 建议反馈 120 条 / 10 分钟，超出返回 429 且不写库。按账号计数、只用现有表，不改表结构、不需要 Redis；要调就改文件顶部那三个配额常量。**已知取舍**：`/api/ai/suggestion-feedback` 是前端即发即忘调用的（`void api.xxx()`），触发 429 时用户看不到提示，只会静默丢一次训练语料——所以这一档限额明显更宽。
...
改频控时看 `tests/test_ratelimit.py`：`QuotaConstantTest` 锁住三个限额常量的量级（手滑改成 `limit=1` 会红），其余用 `dataclasses.replace` 把限额临时改小来测机制，所以调整限额数字**不需要**改测试。
...
语音录制与转写、年度回顾、毕业声音地图、系统推送、图片内容识别、外接自动审核服务与人工复核队列、分布式限流（Redis 计数器 / 设备指纹 / 账号画像）、审核中状态机与分级处置、密码找回、正式云部署和发布签名。当前适合局域网功能验证，图片仍保存在本地目录。
